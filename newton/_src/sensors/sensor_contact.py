...
class SensorContact:
    ...
    def __init__(
        ...
        Args:
            ...
            request_state_attributes: If True (default), transparently request the extended state attribute ``contact_f`` from the model.
        """
        ...
        # request contact force state attribute
        if request_state_attributes:
            model.request_state_attributes("contact_f")
    ...
    def eval(self, contacts: Contacts, state: State):
        ...
        Args:
            ...
            state (State): The state containing contact forces (must have contact_f attribute).
        """
        if state.contact_f is None:
            raise ValueError(
                "SensorContact requires a State with contact_f allocated. Create SensorContact before State."
            )
        self._eval_net_force(contacts, state.contact_f)
    ...
    def _eval_net_force(self, contacts: Contacts, contact_f: wp.array):
