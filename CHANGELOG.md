...
- Fix `collision_filter_parent` being silently ignored on joints to world (`parent=-1`). Explicit values are now honored, and the default for `add_joint_fixed(parent=-1, ...)` auto-filters child-body shapes against world-static shapes (e.g. ground plane)
