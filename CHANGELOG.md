...
- Add `ModelBuilder.clone_builders()` to construct heterogeneous worlds from a caller-provided clone plan in one batched pass. Each plan row becomes one world with the selected source builders added in order, with per-occurrence transforms and label prefixes, and the call records prototype/clone provenance (`ModelBuilder.prototype_label`, `prototype_counts`, `clone_prototype`, `clone_label`, `clone_starts`) for translating prototype-local indices into final builder and model indices. `ModelBuilder.collapse_fixed_joints()` raises once clone metadata exists; collapse fixed joints on the prototypes before cloning.
